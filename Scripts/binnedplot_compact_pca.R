...
plot_pca_megafigure <- function(Cov, B_start, method, wAxisy, wAxisx){
  ...
  for(d in 1:length(modeldata)){
    ...
    partialmod = paste(setdiff(pca.list, Cov), collapse = "+")
    ...
    statsAcc$bcolour <- ifelse(statsAcc$sig>=.05, "grey", ifelse(statsAcc$b < 0, "red", "blue"))
    statsAcc$b <- paste("b =", rd(statsAcc$b,3))
    statsAcc$issig <- ifelse(statsAcc$sig<.05, "*", "")
    ...
    statsRT$bcolour <- ifelse(statsRT$sig>=.05, "grey", ifelse(statsRT$b < 0, "red", "blue"))
    statsRT$b <- paste("b =", rd(statsRT$b,3))
    statsRT$issig <- ifelse(statsRT$sig<.05, "*", "")
...
for (cov in pca.list){
  if (cov %in% colnames(df.list[[1]])){
    print(cov)
    covPlts = try(plot_pca_megafigure(Cov=cov, B_start=4, method = "discrete", wAxisy = c("VLD_AP_H-F"),wAxisx = pca.list[length(pca.list)]))
...
allAccPlts1[1:rlen] = lapply(1:rlen, function(idx){arrangeGrob(allAccPlts1[[idx]], top=textGrob(modeldata[idx], gp = gpar(fontsize = 24)))})
accp <- arrangeGrob(grobs = allAccPlts1, ncol = rlen, 
                    left = textGrob("Scaled Residuals of Accuracy (proportion) by Principle Components Regression", gp=gpar(fontsize = 22), rot=90),
...
rtp <- arrangeGrob(grobs = allRTPlts1, ncol = rlen, 
                   left = textGrob("Residuals of Reaction Time (ms) by Principle Components Regression", gp=gpar(fontsize = 22), rot=90),
                   bottom = textGrob("NOS", gp=gpar(fontsize = 22)),
                   widths = WList, heights = HList)

w <- 3.5 * length(df.list) #width and height of compiled plots
h <- 4 * length(pgCovs)

ggsave(paste(subfolder, "\\", dflistname, "pca_accp", ".png", sep=""), accp, height = h, width = w) 
ggsave(paste(subfolder, "\\", dflistname, "pca_rtp", ".png", sep=""), rtp, height = h, width = w)
