...
accp <- arrangeGrob(grobs = allAccPlts1, ncol = rlen, 
                    left = textGrob("Scaled Residuals of Accuracy by Principle Components Regression", gp=gpar(fontsize = 22), rot=90),
...
rtp <- arrangeGrob(grobs = allRTPlts1, ncol = rlen, 
                   left = textGrob("Residuals of Reaction Time by Principle Components Regression", gp=gpar(fontsize = 22), rot=90),
                   bottom = textGrob("NOS", gp=gpar(fontsize = 22)),
                   widths = WList, heights = HList)
