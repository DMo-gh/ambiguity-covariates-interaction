...
plot_cov_megafigure <- function(Cov, B_start, method, wAxisy, wAxisx){
...
for (cov in covariate.list){
  if (cov %in% colnames(df.list[[1]])){
    print(cov)
    covPlts = try(plot_cov_megafigure(Cov=cov, B_start=4, method = "discrete", wAxisy = c("VLD_AP_H-F"),wAxisx = bottomCovs))
...
for (pg in 1:pgs){
  ...
  HList = c(3.4, rep(3, length(pgCovs)-2), 3.5)
  ...
  allRTPlts1[1:rlen] = lapply(1:rlen, function(idx){arrangeGrob(allRTPlts1[[idx]], top=textGrob(modeldata[idx], gp = gpar(fontsize = 26)))})
  
  rtp <- arrangeGrob(grobs = allRTPlts1, ncol = rlen, 
                      left = textGrob("Reaction Time (ms)", gp=gpar(fontsize = 22), rot=90),
                      bottom = textGrob("NOS", gp=gpar(fontsize = 22)),
                      widths = WList, heights = HList)
  ...
  ggsave(paste(subfolder, "\\", dflistname, "simple_accp", pg, ".png", sep=""), accp, height = h, width = w) 
  ggsave(paste(subfolder, "\\", dflistname, "simple_rtp", pg, ".png", sep=""), rtp, height = h, width = w)
